 ...
 */
function determineCoordinateSign(coord: string) {
...
function isValidDMSCoordinates(coordinateParts: number[], isLatitude: boolean) {
 ...
 */
function parseCoordinateStringToDegreesMinutesSeconds(coord: string) {
  ...
  const decimal = convertStringToDecimal(decimalPart || "");
...
function convertDMSToDecimalDegrees(coordinateParts: number[]) {
 ...
 */
function isCoordinateStringValid(coord: string) {
 ...
 */
export function convertCoordStringToDecimalDegrees(coord: string) {
  ...
  if (!isCoordinateStringValid(trimmedCoordinate)) return null;

  const coordinateDirectionMultiplier =
    determineCoordinateSign(trimmedCoordinate);
  const coordWithoutDirection = trimmedCoordinate
    .replace(/[NSEW+-]/gi, "")
    .trim();

  const coordinateParts = parseCoordinateStringToDegreesMinutesSeconds(
    coordWithoutDirection,
  );
  ...
  return (
    convertDMSToDecimalDegrees(coordinateParts) * coordinateDirectionMultiplier
  );
}
